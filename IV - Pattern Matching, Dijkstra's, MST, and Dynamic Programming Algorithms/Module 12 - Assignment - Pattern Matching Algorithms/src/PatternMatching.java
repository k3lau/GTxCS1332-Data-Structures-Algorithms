  ...
  public static List<Integer> boyerMoore(CharSequence pattern, CharSequence text, CharacterComparator comparator) {
    ...
    while (textIndex <= text.length() - pattern.length()) {
      ...
      while (patternIndex >= 0
          && comparator.compare(text.charAt(textIndex + patternIndex), pattern.charAt(patternIndex)) == 0) {
        patternIndex--;
      }
      if (patternIndex == -1) {
        ...
        int shift = table.getOrDefault(text.charAt(textIndex + patternIndex), -1);
        if (shift < patternIndex) {
          textIndex = textIndex + patternIndex - shift;
        } else {
          textIndex++;
        }
      }
